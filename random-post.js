...
(function(a, b, c) {
    ...
    do_related_post_start = function(json) {
        ...
        inject(def.homePage + '/feeds/posts/summary?alt=json-in-script&orderby=updated&start-index=' + si + '&max-results=' + def.numPosts + '&callback=do_related_post_' + hash);
    },
    ...
    do_related_post = function(json) {
        var ct = b.getElementById(def.containerId),
            ...
            st = def.widgetStyle,
            sk = def.widgetTitle + '<ul class="clearfloat related-post-style-' + st + '">',
        ...
        for (var i = 0; i < def.numPosts; i++) {
            ...
            img = ("media$thumbnail" in entry[i] && def.thumbnailSize !== false) ? entry[i].media$thumbnail.url.replace(/\/s\d+(\-c)?\//, '/w' + def.thumbnailSize + '-' + 'h' + def.thumbnailHeight + '-c/') : def.noImage;
            ...
            if (st == 2) {
                ...
                sk += '<li class="' + ci + '" tabindex="0"><a class="' + ci + '-titleimg" href="' + link + '"' + tg + '><img alt="" class="' + ci + '-thumbnail" src="' + img + '" width="' + def.thumbnailSize + '" height="' + def.thumbnailHeight + '"></a><div class="' + ci + '-tooltip"><a class="' + ci + '-title" title="' + tt + '" href="' + link + '"' + tg + '>' + title + '</a></div>' + cl + '</li>';
            } else {
